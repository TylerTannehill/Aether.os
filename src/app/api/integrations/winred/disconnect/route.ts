...
export async function POST() {
  try {
    ...
    if (appUserError) {
      console.error("WinRed disconnect Aether user lookup failed", appUserError);
    ...
    if (membershipError) {
      console.error(
        "WinRed disconnect membership lookup failed",
        membershipError
      );
    ...
    if (!existingConnection) {
      return NextResponse.json({
        ...
        connected: false,
        provider: PROVIDER,
        message: "WinRed is already disconnected.",
      });
    }
    ...
    return NextResponse.json({
      ...
      provider: PROVIDER,
      organizationId,
      message: "WinRed disconnected.",
    });
  } catch (error: any) {
    console.error("WinRed disconnect failed", error);

    return NextResponse.json(
      {
        success: false,
        error: error?.message || "Failed to disconnect WinRed.",
      },
      { status: 500 }
    );
  }
