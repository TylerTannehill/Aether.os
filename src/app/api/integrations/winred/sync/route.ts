  ...
  try {
    ...
    const winRedContacts = [
      ...
      },
      {
        ...
        phone: "2175550166",
        address: "91 W Adams St",
        city: "Springfield",
        state: "IL",
        occupation: "Executive",
        employer: "Bennett Holdings",
        fec_total_given: 3300,
        fec_recent_activity: true,
        ...
        jackpot_reason:
          "High-value WinRed donor should be reviewed for finance follow-up.",
      },
    ];

    const normalizedContacts = winRedContacts.map((contact) => ({
      organization_id: organizationId,
    ...
    return NextResponse.json(
      {
        success: false,
        source: "winred",
        error: error?.message || "WinRed sync failed.",
      },
      { status: 500 }
    );
  }
