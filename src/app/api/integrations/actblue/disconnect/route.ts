...
export async function POST() {
  try {
    ...
    if (appUserError) {
      console.error("ActBlue disconnect Aether user lookup failed", appUserError);
    ...
    if (membershipError) {
      console.error(
        "ActBlue disconnect membership lookup failed",
        membershipError,
      );
    ...
    if (!existingConnection) {
      return NextResponse.json({
        ...
        connected: false,
        provider: PROVIDER,
        message: "ActBlue is already disconnected.",
      });
    }
    ...
    return NextResponse.json({
      ...
      provider: PROVIDER,
      organizationId,
      message: "ActBlue disconnected.",
    });
  } catch (error: any) {
    console.error("ActBlue disconnect failed", error);

    return NextResponse.json(
      {
        success: false,
        error: error?.message || "Failed to disconnect ActBlue.",
      },
      { status: 500 },
    );
  }
