...
export async function GET() {
  try {
    ...
    if (appUserError) {
      console.error("Google Routes status Aether user lookup failed", appUserError);
    ...
    if (membershipError) {
      console.error("Google Routes status membership lookup failed", membershipError);
    ...
    const connected = Boolean(connection?.status === "connected");
    ...
    console.error("Google Routes status failed", error);

    return NextResponse.json(
      {
        success: false,
        error: error?.message || "Failed to load Google Routes status.",
      },
      { status: 500 },
    );
  }
